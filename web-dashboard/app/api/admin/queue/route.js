...
export async function GET(request) {
    ...
    try {
        ...
        const [queued] = await connection.execute(
            `SELECT d.id, d.title, d.brand, d.category, d.url, d.status, d.is_fb_posted, d.is_blog_posted, d.created_at 
             FROM normalized_deals d 
             WHERE d.status = 'approved' AND d.is_blog_posted = FALSE
             ORDER BY d.created_at ASC`
        );

        // 3. Recently Published (deals that have blogs)
        const [published] = await connection.execute(
            `SELECT d.id, b.title as blog_title, d.title, d.category, d.url, d.is_fb_posted, d.is_blog_posted, b.created_at as published_at 
             FROM normalized_deals d 
