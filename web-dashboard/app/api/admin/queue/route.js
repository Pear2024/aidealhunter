...
export async function GET(request) {
    ...
    try {
        ...
        const [queued] = await connection.execute(
            `SELECT d.id, d.title, d.brand, d.category, d.url, d.status, d.created_at 
             FROM normalized_deals d 
             LEFT JOIN ai_blog_posts b ON d.id = b.source_deal_id 
             WHERE d.status = 'approved' AND b.id IS NULL 
             ORDER BY d.created_at ASC`
        );

        // 3. Recently Published (deals that have blogs)
        const [published] = await connection.execute(
            `SELECT d.id, b.title as blog_title, d.title, d.category, d.url, b.created_at as published_at 
             FROM normalized_deals d 
